...
# --- DEDICATED LANGUAGE/COPY QUALITY GATE ------------------------------------
# Root cause this fixes: llm_quality_issues() above already has good grammar/
# naturalness codes ("unnatural_language", "explains_instead_of_shows",
# "generic_ai_ad") — but it only ever RUNS when Layer 1's deterministic
# checks (deterministic_issues, called from _apply_quality_gate) found
# NOTHING first. Layer 1 fires often (a banned phrase, the filmability
# density check, a missing field) on scripts that are otherwise fine content-
# wise, and every time it does, the semantic language check is skipped
# entirely — so a rewrite driven by an unrelated Layer-1 issue has ZERO
# verification of its own grammar/naturalness. This gate is deliberately
# UNCONDITIONAL (never short-circuited by any other gate's result) and
# focused on ONLY writing quality — nothing about claims, structure,
# territory, or product relevance, which stay owned by the other gates —
# and it runs on the FINAL, fully-settled text (see script_service.py's
# _apply_language_quality_gate, positioned last in the gate sequence).
_LANGUAGE_QUALITY_SYSTEM_PROMPT = """You are a senior Indian advertising copy editor doing a FINAL
line-by-line pass on a short-form ad script, immediately before it ships. You are NOT judging the
creative idea, the claims, the structure, or the product fit — those have already been checked
elsewhere. Your ONLY job is whether the ACTUAL WORDING is something a professional Indian advertising
copywriter would sign off on without rewriting it, sentence by sentence.

For EVERY line (hook, each body line, CTA), silently apply the READ-ALOUD TEST: "Would a real Indian
person naturally say this exact sentence out loud?" If no, the line fails, even if it is grammatically
parseable and technically communicates the idea.

Concrete examples of the exact failure pattern to catch — reject THIS STYLE of writing, not just these
exact words:
- "Aadat sirf packet nahi; woh reach, break aur familiar taste ka poora ritual hai." — a strategist
  explaining a concept in a semicolon-joined sentence, not a person talking.
- "Choice badli — switching habit ko support mila." — two noun phrases mechanically mashed together,
  no natural rhythm in either language.
- "Familiar chew ko refreshing banata hai." — English words dropped into a Hindi sentence frame like a
  literal translation, not natural code-switching (a real bilingual speaker switches at phrase
  boundaries, not mid-noun-phrase).

Check, across the whole script:
- GRAMMAR (English, Hindi, and Hinglish each on their own terms — a Hinglish sentence must be
  grammatically sound AS Hinglish, not just as two languages glued together)
- SENTENCE CONSTRUCTION AND FLOW — does each sentence read the way a person actually builds a
  sentence, not a translated or mechanically assembled one?
- DIALOGUE AUTHENTICITY — if a line is spoken by a character, does it sound like that specific person
  reacting/asking/interrupting in the moment, or like a marketer/product description/AI explanation
  wearing a character's name?
- WORD CHOICE — natural, register-appropriate words, not stiff or overly formal substitutes for how
  people actually talk.
- PUNCTUATION — natural spoken pauses (short sentences, ellipses, dashes where a person would actually
  pause), not essay-style semicolons or comma-spliced explanations.
- UNNECESSARY REPETITION — the same word/phrase/sentence structure repeated across lines with no
  rhythmic purpose.
- AI/TRANSLATED-SOUNDING LANGUAGE — any line that reads like it was composed in English and translated,
  or generated by an AI summarizing a concept rather than someone writing dialogue.
- AD-COPY RHYTHM — do the beats have the punchy, varied-length rhythm of real short-form ad copy, or
  do they read like uniform explanatory sentences?
- CTA QUALITY — does the closing line sound like an actual spoken/written close a brand would use, not
  a generic template phrase merely swapped for this product?

Do NOT penalize simplicity — a short, plain, natural line is never a defect. Do NOT flag a line just
because it's short or because dialogue is casual; only flag genuine unnaturalness per the read-aloud
test above. Most scripts that reach this stage are close — don't invent problems to seem thorough.

Return ONLY issue codes from this exact list:
- "bad_grammar": a specific grammatical error in English, Hindi, or Hinglish
- "unnatural_phrasing": reads like a stiff translation or mechanically assembled sentence rather than
  something a person would naturally say — the exact failure pattern in the examples above
- "robotic_dialogue": a character's line sounds like a marketer/product description/AI explanation
  rather than a person reacting/speaking in the moment
- "weak_cta_copy": the closing line is a generic template phrase, not an earned, natural-sounding close
- "repetitive_wording": the same word/phrase/construction repeated with no rhythmic purpose
- "awkward_punctuation": semicolons, comma-splices, or essay-style punctuation instead of natural
  spoken pauses

Return ONLY this JSON, no prose, no markdown fences:
{"pass": boolean, "issues": [string], "worst_line": string}
worst_line: quote the single worst-offending line verbatim (empty string if pass=true) — this is what
a human editor would point to first."""


def _build_language_quality_user_message(data: dict, payload) -> str:
    hook_text = (data.get("hook") or {}).get("text", "") if isinstance(data.get("hook"), dict) else ""
    body_texts = [b.get("text", "") for b in (data.get("body") or []) if isinstance(b, dict)]
    cta_text = (data.get("cta") or {}).get("text", "") if isinstance(data.get("cta"), dict) else ""
    return (
...
def llm_language_quality_issues(data: dict, payload) -> tuple[list[str], str]:
    """The dedicated language/copy quality check — see the module comment
    above for why this is a SEPARATE, unconditional call rather than folded
    into llm_quality_issues(). Returns (issues, worst_line) — worst_line is
    "" on a clean pass or on any failure to call/parse. Never raises: a
    failed call is treated as a pass (fail-open, same convention as every
    other gate here) — this catches a real defect, it is not itself allowed
    to become a new source of failure."""
    try:
        text = call_openrouter_with_retry(
            lambda: generate_text(
                system_instruction=_LANGUAGE_QUALITY_SYSTEM_PROMPT,
                contents=[_build_language_quality_user_message(data, payload)],
                model=settings.validation_model,
                max_output_tokens=350,
                json_mode=True,
                label="language_quality_gate",
            ),
            label="language_quality_gate",
            max_attempts=2,
        )
        ...
        worst_line = str(result.get("worst_line") or "")
        return issues, worst_line
    except Exception as e:
        logger.warning("Language-quality gate LLM evaluation failed, treating as pass: %s", e)
        return [], ""
...
_ISSUE_INSTRUCTIONS: dict[str, str] = {
    ...
    # --- Dedicated language/copy quality gate codes (see llm_language_quality_issues) ---
    "bad_grammar": (
        "one or more lines have a specific grammatical error (English, Hindi, or Hinglish) — fix the "
        "grammar directly, keeping the same meaning and tone"
    ),
    "unnatural_phrasing": (
        "one or more lines read like a stiff translation or a mechanically assembled sentence rather "
        "than something a real person would naturally say out loud — rewrite them directly in the "
        "natural spoken register (do not translate sentence-by-sentence); avoid semicolon-joined "
        "explanatory sentences and English/Hindi phrases mashed together mid-noun-phrase — real "
        "code-switching happens at natural phrase boundaries"
    ),
    "robotic_dialogue": (
        "a character's line sounds like a marketer, a product description, or an AI explaining the "
        "concept rather than a real person reacting, asking, hesitating, or speaking in the moment — "
        "rewrite it as something that specific person would actually say, not a summary of the idea"
    ),
    "weak_cta_copy": (
        "the closing line is a generic template phrase rather than an earned, natural-sounding close — "
        "rewrite it to sound like something this brand would actually say, matching the story's tone"
    ),
    "repetitive_wording": (
        "the same word, phrase, or sentence construction repeats across lines with no rhythmic purpose "
        "— vary the wording/structure so each line sounds distinct"
    ),
    "awkward_punctuation": (
        "semicolons, comma-splices, or essay-style punctuation are used instead of natural spoken "
        "pauses — break these into short, punchy lines the way a person would actually pause when "
        "speaking, not how an essay is punctuated"
    ),
}
